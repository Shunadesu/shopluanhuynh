# Implementation Guide: Thống kê chính sách

## 🎯 Quick Implementation (Ready to Code)

### Step 1: Enable Menu Item

**File**: `frontend/src/components/AccountSidebar.jsx:28-33`
...
### Step 2: Add Route Handler in Profile.jsx

**File**: `frontend/src/pages/Profile.jsx`

**Line 102** - Add constant:
```jsx
const isPoliciesView = viewParam === 'policies';
```

**Line 131** - Update SEO title:
```jsx
const seoTitle = isOrderDetailView
  ...
  : isPoliciesView ? 'Dashboard & Thống kê'  // ✅ Add this
  : 'Tài Khoản Của Tôi';
```

**Line 150** - Add routing logic:
```jsx
{isDepositView ? (
  ...
  isLoading ? <ContentSkeleton /> : <PoliciesSection user={user} />  // ✅ Add this
...
### Step 3: Create PoliciesSection Component
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          icon={FiCreditCard}
          label="Đã nạp tiền"
          value={`${(stats.totalDeposited / 1000).toFixed(1)}k`}
          ...
          <div className="space-y-2">
            {stats.categories.map(([category, count], idx) => (
              ...
              >
                ...
                <span className="text-primary font-bold">{count} tài khoản</span>
              </div>
            ))}
          </div>
        </div>
...
const FinancialBar = ({ label, value, color, icon: Icon }) => {
  return (
    <div>
      ...
      <div className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
        <div className={`h-full ${color} rounded-full transition-all`} style={{ width: '75%' }} />
      </div>
    </div>
  );
};
...
- [ ] Bỏ `disabled: true` trong AccountSidebar
- [ ] Thêm `isPoliciesView` constant trong Profile.jsx
- [ ] Thêm routing logic cho PoliciesSection
- [ ] Tạo file PoliciesSection.jsx
- [ ] Import PoliciesSection trong Profile.jsx
- [ ] Test navigation từ sidebar
- [ ] Kiểm tra stats hiển thị đúng
- [ ] Test responsive trên mobile
- [ ] Verify dark mode hoạt động
- [ ] Test với empty data (no orders/deposits)
...
┌────────────┬────────────┬────────────┬────────────┐
│ 🛒 Tổng ĐH │ 💰 Đã chi  │ 💳 Đã nạp  │ 📊 Số dư   │
│    25      │   2.5M     │    3.0M    │   500k     │
│ 20 hoàn    │ 2,500,000đ │ 15 giao d. │ +500,000đ  │
└────────────┴────────────┴────────────┴────────────┘
...
┌──────────────────────┬──────────────────────────┐
│ 📦 Phân tích đơn hàng │ 💳 Phương thức nạp tiền  │
│ ✓ Hoàn thành: 20     │ 🏦 Ngân hàng: 12         │
│ ⏱ Đang xử lý: 3      │ 📱 Thẻ cào: 3            │
│ ✕ Đã hủy: 2          │                          │
└──────────────────────┴──────────────────────────┘
```
...
# Test at: http://localhost:5173/profile?view=policies
```
...
- Add bar chart for deposit vs spending comparison
